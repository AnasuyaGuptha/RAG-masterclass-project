...
12. If multiple policies are retrieved,
answer only from the policy relevant
to the question.
...
using Zyro Dynamics HR policy documents.
...
if prompt := st.chat_input(
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner(
            ...
            if (
                guard_result.strip().upper()
                != "IN_SCOPE"
            ):

                answer = (
                    "I can only answer questions "
                    "about Zyro Dynamics HR policies "
